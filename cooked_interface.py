...
if model.loadModel("my_model"):
	...
	MainInputCol2 = [
		...
		[sg.Text("Year:", background_color="#B1A7A6", text_color="black"),
		 ...
		 sg.InputText('2018', key="year-input", enable_events=True, size=(6, 1), background_color="#D3D3D3",
					  text_color="black")],
		# max_power
		...
		[sg.Text("Box:", background_color="#B1A7A6", text_color="black"),
		 sg.Combo(['robot', 'automatic', 'variator', 'mechanics'], size=(41, 1), key="box",
				  background_color="#D3D3D3", text_color="black", button_background_color="#F5F3F4",
				  button_arrow_color="black", default_value='variator')]
	]
	...
	SecondaryInputCol1 = [
		...
		[sg.Text("Torgue:", background_color="#B1A7A6", text_color="black"),
		 ...
		 sg.InputText('380', key="torque-input", enable_events=True, size=(6, 1), background_color="#D3D3D3",
					  text_color="black")],
		# speed_to_100
		[sg.Text("Speed to 100:", background_color="#B1A7A6", text_color="black"),
		 sg.Slider(orientation="horizontal", enable_events=True, key="speed_to_100-slider", range=(0.00, 50),
				   size=(22, 13),
				   trough_color="#D3D3D3", background_color="#B1A7A6", text_color="black", default_value=9.5),
		 sg.InputText('9.5', key="speed_to_100-input", enable_events=True, size=(6, 1), background_color="#D3D3D3",
					  text_color="black")]
	]
	...
	SecondaryInputCol2 = [
		...
		[sg.Text("Maximum speed:", background_color="#B1A7A6", text_color="black"),
		 sg.Slider(orientation="horizontal", enable_events=True, key="maximum_speed-slider", range=(10.0, 492.0),
				   size=(19, 13),
				   trough_color="#D3D3D3", background_color="#B1A7A6", text_color="black", default_value=199),
		 sg.InputText('199', key="maximum_speed-input", enable_events=True, size=(6, 1), background_color="#D3D3D3",
					  text_color="black")],
		# consumption
		[sg.Text("Consumption:", background_color="#B1A7A6", text_color="black"),
		 ...
		 sg.InputText('6.2', key="consumption-input", enable_events=True, size=(6, 1), background_color="#D3D3D3",
					  text_color="black")]
	]
	...
	Price = [[sg.Output(size=(101, 3), key='-OutputText-',
						background_color="#D3D3D3", text_color="black", sbar_background_color="#F5F3F4",
						sbar_arrow_color="black")]]

	# Фрейм со всеми полями ввода
	Input = [
		[sg.Frame('Ввод основных атрибутов', layout=MainInput, border_width=6, background_color="#B1A7A6",
				  title_color="black", font="bold")],
		[sg.Frame('Ввод дополнительных атрибутов', layout=SecondaryInput, border_width=6, background_color="#B1A7A6",
				  title_color="black", font="bold")],
		[sg.Button('Принять', enable_events=True, key='-LoadButton-', size=(81, 2),
				   border_width=4, button_color=('#BA181B'), font='Helvetica 13')],
		[sg.Frame('Цена автомобиля', layout=Price, border_width=6, background_color="#B1A7A6", title_color="black",
				  font="bold")]
	]

	# Кнопки для вывода графиков
	GraphButtons = [[sg.Button('График1', enable_events=True, key='-Graph1-', size=(33, 3),
							   border_width=1, button_color=('#BA181B'), font='Helvetica 11'),
					 sg.Button('График2', enable_events=True, key='-Graph2-', size=(33, 3),
							   border_width=1, button_color=('#BA181B'), font='Helvetica 11')]]

	Graphs = [[sg.Frame('Графики', layout=GraphButtons, border_width=9, background_color="#151314", title_color="white",
						font="bold")]]

	# Кнопка выхода
	ExitButton = [[sg.Button('Выйти', enable_events=True, key='-Exit-', size=(8, 3), button_color=('#A4161A'),
							 font='Helvetica 14')]]
	...
	layout = [[sg.Frame('', layout=Input, border_width=9, background_color="#B1A7A6", title_color="black")],
			  [sg.Column(Graphs, element_justification='c', background_color="#151314"),
			   sg.Column(ExitButton, element_justification='c', background_color="#151314")]]
	...
	while True:
		...
		if event == '-LoadButton-':
			...
			if (values['mark'] == "" or values['model'] == "" or values['car_class'] == "" or
				...
				data = [values['mark'],
						values['model'],
						values['mileage-slider'],
						values['year-slider'],
						values['car_class'],
						values['max_power-slider'],
						values['drive_unit'],
						values['box'],
						values['boost_type'],
						values['volume-input'],
						values['torque-slider'],
						values['maximum_speed-slider'],
						values['speed_to_100-input'],
						values['consumption-input']]
